...
export default function PartenaireDetailPage() {
  ...
  const partner: PartnerData | undefined = partenaires[slug as string];
            ...
            {partner.accreditationImage && (
              <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
                <div className="bg-white rounded-2xl border-2 border-penn-border/20 p-8 shadow-sm">
                  <img src={partner.accreditationImage} alt="Accréditations" className="w-full h-auto max-h-[100px] object-contain mx-auto" />
                </div>
              </motion.div>
            )}
                ...
                >
                  {acc.logo && (
                    <div className="h-16 flex items-center justify-center mb-3">
                      <img src={acc.logo} alt={acc.label} className="max-h-full max-w-[180px] object-contain mx-auto" onError={(e) => { (e.target as HTMLElement).style.display = "none"; }} />
                    </div>
                  )}
                ...
                <motion.div key={t.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} className="bg-penn-bg-light rounded-2xl p-6 border border-penn-border/50">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 shrink-0 relative">
                      <img src={t.photo} alt={t.name} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLElement).style.display = "none"; }} />
                    </div>
                    <div>
                  ...
                  <div className="bg-gradient-to-br from-penn-bg-light to-white rounded-xl p-4 border-2" style={{ borderColor: `${accent}20` }}>
                    ...
                    <div className="flex items-center gap-2.5">
                      <div className="w-9 h-9 rounded-full overflow-hidden bg-gray-200 shrink-0 border-2 border-white shadow-sm">
                        <img src={block.testimonial.photo} alt={block.testimonial.name} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLElement).style.display = "none"; }} />
                      </div>
                      <div>
