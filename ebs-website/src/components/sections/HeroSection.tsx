...
export default function HeroSection({ certificationCount }: { certificationCount: number }) {
  ...
  const courseCategories = [
    { title: t('categories.Licence'), count: siteStats.licenceDegrees, icon: GraduationCap, badgeColor: "bg-[#f4a261]", href: "/licences" },
    { title: t('categories.Master'), count: siteStats.masterDegrees, icon: Award, badgeColor: "bg-[#84cc16]", href: "/masters" },
            ...
            <div className="py-10 max-md:py-0 relative">
              ...
              >
                <div className="flex flex-col items-end mb-2 relative z-20 pr-4 md:pr-8">
                  <span className="text-[16px] md:text-[18px] text-penn-green font-semibold italic text-right leading-tight w-[200px]">
                    {t('sliderTitle')}
                  </span>
                  <svg width="45" height="45" viewBox="0 0 100 100" fill="none" className="text-penn-green mr-6 md:mr-12 mt-1">
                        ...
                        <CarouselItem key={idx} className="pl-4 basis-[140px] md:basis-[160px]">
                          <Link href={cat.href as React.ComponentProps<typeof Link>["href"]} className="w-full h-[150px] md:h-[160px] relative bg-transparent border border-dashed border-[#a3a3a3] flex flex-col items-center justify-center p-3 md:p-4 transition-all duration-300 hover:border-penn-green cursor-pointer group">
                            ...
                            <div className="mb-2 md:mb-3">
                              <cat.icon className="w-10 h-10 md:w-12 md:h-12 text-[#42474c] stroke-[1px] group-hover:text-penn-green transition-colors" />
                            </div>
                            {/* Text */}
                            <h3 className="text-center font-bold text-[#232434] text-[12px] md:text-[13px] leading-snug group-hover:text-penn-green transition-colors">
                              {cat.title}
                            </h3>
                          </Link>
                        </CarouselItem>
