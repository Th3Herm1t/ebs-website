...
export function generateStaticParams() {
  return ["fr", "en"].flatMap((locale) =>
    Object.keys(getMasters()).map((slug) => ({ locale, slug }))
  );
}

export async function generateMetadata({ params }: PageParams) {
  const { slug, locale } = await params;
  const data = getMasters(locale === "en" ? "en" : "fr")[slug];
  if (!data) return {};
...
export default async function MasterLPPage({ params }: PageParams) {
  ...
  const breadcrumb = breadcrumbJsonLd([
     ...
     { name: "Masters", path: locale === "en" ? "/en/masters" : "/masters" },
     { name: data.title, path: `${locale === "en" ? "/en" : ""}/masters/${data.slug}` },
  ]);
